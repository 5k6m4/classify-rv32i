...
The main issue encountered in this project is how to perform multiplication without using M extension instructions. I attempted to solve this problem using addition and shifting, and a detailed explanation is as follows.

Suppose we want to calculate the product of two 4-bit integers, `-3` and `5`, where `-3` is the multiplicand and `5` is the multiplier. The binary representations of `-3` and `5` are `0b1101` and `0b0011`, respectively. In long multiplication, multiplying two numbers essentially involves repeated shifting and accumulating, as shown in the diagram below.
```
                1 1 0 1
    x)          0 0 1 1
  -----------------------
  ...
  -----------------------
        0 0 1 0 0 1 1 1
```
Therefore, to compute the product of these two numbers, we decide whether to add the multiplicand's value to the result based on whether the least significant bit (LSB) of the multiplier is 1. After checking the current LSB, we shift the multiplier one bit to the right to check each bit sequentially. At the same time, we shift the multiplicand one bit to the left, achieving the effect of adding multiples of the multiplicand with different powers of two, similar to long multiplication.

However, after I finished the assignment, I realized that I might have overcomplicated things. Actually, I could just repeatedly add the multiplier to achieve the multiplication effect. If I have time later, I will come back and update the multiplication implementation with this new idea, and compare the differences between the two methods.

## Result
After entering `bash test.sh all`, you can see the message below in the terminal, indicating that all the tests have passed.
```
